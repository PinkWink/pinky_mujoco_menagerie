...
from utils.lidar_visualizer import LidarVisualizer 
...
class PinkyMuJoCo:
    def __init__(self):
        ...
        self.simulator = MuJoCoViewer(self.model, self.data)
        
        # 2. LidarVisualizer 객체 생성
    ...
    def start_rendering(self):
        try:
            while not self.simulator.should_close():
                ...
                # 3. 렌더링 루프에서 visualizer 업데이트
                self.lidar_viz.update(self.model, self.data, "laser")
            ...
            # 4. 시뮬레이션 종료 시 LidarVisualizer 창도 함께 닫기
            if self.lidar_viz:
